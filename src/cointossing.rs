...
fn test_coinshare() {
    ...
    assert_eq!(expected, finish(coin_share1, commitment_msg2, coin_msg2).unwrap());
    assert_eq!(expected, finish(coin_share2, commitment_msg1, coin_msg1).unwrap());
}
...
fn test_coinshare_fail() {
    ...
    // randomly corrupt the coin value by 1 bit and check that the protocol fails
    {
        ...
        assert_eq!(
            Err(Error::MacError),
            finish(coin_share1.clone(), commitment_msg2_nok, coin_msg2_ok.clone())
        );

        assert_eq!(
            Err(Error::MacError),
            finish(coin_share1.clone(), commitment_msg2_ok, coin_msg2_nok.clone())
        );
    }
}
