...
use crate::{
    ...
    Gate, GateIndex,
    cointossing::{
        self, {CoinResult, CoinShare},
    },
    hash::{garbling_hash, hash, hash_key, hash_keys},
    leakyand::{compute_leaky_and_hashes, derive_and_shares},
    leakydelta_ot::{
        BLOCK_SIZE, LeakyOtReceiver, LeakyOtSender, ReceiverInitializer, SenderInitializer,
        message::{OtInitReply, SerializedOtInit},
    },
    types::{
        AndTableShare, BitShare, Delta, InputMaskShare, K, KeyType, MacType, PartialBitShare,
        TableShare, WireLabel, WireMask, WireState,
    },
};
...
impl<C: Borrow<Circuit>, I: Borrow<[bool]>> Contributor<C, I> {
    ...
    pub fn new(circuit: C, input: I, rng: ChaCha20Rng) -> Result<(Self, Msg), Error> {
        ...
        let contrib = Self { state: Box::new(ContribState::Step1(state)), circuit, input };
        Ok((contrib, msg))
    }
    ...
    pub fn run(self, msg: &[u8]) -> Result<(Contributor<C, I>, Msg), Error> {
        ...
        let next_state = Contributor { state, circuit: self.circuit, input: self.input };
        Ok((next_state, msg))
    }
}
...
impl<C: Borrow<Circuit>, I: Borrow<[bool]>> Evaluator<C, I> {
    ...
    pub fn new(circuit: C, input: I, rng: ChaCha20Rng) -> Result<Self, Error> {
        ...
        Ok(Self { state: Box::new(EvalState::Step1(state)), circuit, input })
    }
    ...
    pub fn run(self, msg: &[u8]) -> Result<(Evaluator<C, I>, Msg), Error> {
        ...
        let next_state = Evaluator { state, circuit: self.circuit, input: self.input };
        Ok((next_state, msg))
    }
...
fn init_ot1(delta: Delta, mut rng: ChaCha20Rng, p: &Circuit) -> StateResult<OtInitState1> {
    ...
    let triples_bits_aligned = triples_bits.div_ceil(TRIPLES) * TRIPLES;
    let total_abits = wire_abits + triples_bits_aligned;
    let num_abits_aligned = total_abits.div_ceil(BLOCK_SIZE) * BLOCK_SIZE;
    ...
    let state =
        OtInitState1 { rng, delta, r_init, coin_share, blocks: num_abits_aligned / BLOCK_SIZE };
    Ok((state, msg))
}
...
fn init_ot3(state: OtInitState2, msg: &[u8]) -> StateResult<OtInitState3> {
    ...
    let coin = cointossing::finish(state.coin_share, state.coin_commitment, upstream_coin)?;
    let ot_init = serialized_ot_init.deserialize()?;
...
fn ot_ands1(mut state: OtInitState4, msg: &[u8], circuit: &Circuit) -> StateResult<OtAndsState1> {
    ...
    for (block_id, block) in blocks.into_iter().enumerate() {
        let ot_rx: [MacType; BLOCK_SIZE] =
            block.try_into().map_err(|_| Error::OtBlockDeserializationError)?;
    ...
    let abits_blocks = n_wire_abits.div_ceil(BLOCK_SIZE);
    ...
    let num_blocks = n_and_gates.div_ceil(BLOCK_SIZE) * BLOCK_SIZE;
...
fn compute_u(delta: &Delta, and_bits: &[BitShare]) -> Vec<MacType> {
    ...
    for i in (0..and_bits.len()).step_by(3) {
        let BitShare { key: k_x1, bit: b_x2, .. } = and_bits[i];
        let BitShare { key: k_y1, bit: b_y2, .. } = and_bits[i + 1];
        let BitShare { key: k_z1, bit: b_z2, .. } = and_bits[i + 2];
        let t0 = hash_keys(k_x1, k_z1 ^ (if b_z2 { delta.0 } else { 0 }));
        let u0 =
            t0 ^ hash_keys(k_x1 ^ delta.0, k_y1 ^ k_z1 ^ (if b_y2 ^ b_z2 { delta.0 } else { 0 }));
...
fn ot_ands4(mut state: OtAndsState3, msg: &[u8]) -> StateResult<OtAndsState4> {
    ...
    for i in (0..and_bits.len()).step_by(3) {
        let BitShare { key: k_x1, mac: m_x2, bit: b_x2 } = and_bits[i];
...
fn ot_ands5(mut state: OtAndsState4, msg: &[u8]) -> StateResult<OtAndsState5> {
    ...
    for i in (0..and_bits.len()).step_by(3) {
        ...
        let t1 = hash_keys(k_x1, k_y1 ^ k_z1 ^ (if b_y2 ^ b_z2 { state.delta.0 } else { 0 }));
...
fn check_hash(
    ...
    if !success { Err(LeakyAndNotEqual) } else { Ok(()) }
}
...
impl AndsBucketingState {
    ...
    fn update_triples(self, msg: &[u8]) -> Result<AndsBucketingState, Error> {
        ...
        Ok(AndsBucketingState { and_triples, bucketing_bits: Default::default(), ..self })
    }
}
    ...
    for (index, gate) in circuit.gates().iter().enumerate() {
        if gate == &Gate::InEval {
            input_mask_shares.push((
                index as GateIndex,
                PartialBitShare { bit: masks[index].bit.bit, mac: masks[index].bit.mac },
            ))
        }
    }
...
fn ot_ands8_eval(
    ...
    let input_gates = circuit.gates().iter().filter(|g| *g == &Gate::InEval).count();
    ...
    fn run(mut self, msg: &[u8], circuit: &Circuit) -> InnerResult<Vec<bool>> {
        ...
        for (index, gate) in circuit.gates().iter().enumerate() {
            if let Gate::Xor(input_lhs, input_rhs) = gate {
                ...
                wires[index].label =
                    wires[*input_lhs as usize].label.xor(&wires[*input_rhs as usize].label);
